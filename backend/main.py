...
DB_PATH = os.path.join(os.path.dirname(__file__), "budgetguard.db")
# For Vercel (read-only filesystem), we might need to use /tmp for sqlite or external DB
# But for now, we'll keep it simple. Note: SQLite resets on redeploy on Vercel.
if os.environ.get("VERCEL"):
    DB_PATH = "/tmp/budgetguard.db"
    # Copy template DB if needed, or init fresh
...
async def simulate_payment(request: SimulatePaymentRequest, token: str):
    ...
    if emergency_zone and user['emergency_fund'] > 0:
        if request.use_emergency:
            if not request.emergency_pin or request.emergency_pin != user['emergency_pin']:
                 return {
                    "approved": False,
                    "reason": "Invalid Emergency PIN",
                    "requires_pin": True,
                    "is_emergency": True
                }
            decision = "APPROVED (EMERGENCY FUNDS)"
            approved = True
        else:
            decision = "WARNING: EMERGENCY PIN REQUIRED"
            approved = False
    elif predicted_spend > user['monthly_budget']:
        decision = "DENIED (OVER BUDGET)"
        approved = False
    else:
        decision = "APPROVED"
        approved = True
    ...
    return {
        "approved": approved,
        "decision": decision,
        "new_budget_usage": round(budget_usage, 1),
        "requires_pin": emergency_zone and user['emergency_fund'] > 0,
        "is_emergency": emergency_zone
    }

@router.post("/check_scam")
...
async def redeem_coins(request: RedeemCoinsRequest, token: str):
    ...
    # Generate mock code
    code = f"{request.brand.upper()}-{secrets.token_hex(4).upper()}"
    
    with get_db() as conn:
        cursor = conn.cursor()
        
        # Deduct coins
        cursor.execute("UPDATE users SET coin_balance = coin_balance - ? WHERE id = ?", 
        ...
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?)
        """, (user['id'], request.brand, request.coins_required, code))
    ...
    return {
        "success": True,
        "code": code,
        "new_balance": user['coin_balance'] - request.coins_required
    }
